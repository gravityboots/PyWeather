...
from weather import getWeather, getForecast, getWeatherMap, localTime
...
class MainFrame(ctk.CTkFrame):
    ...
    def populate(self,c,icons,bfont,mfont,sfont,efont):
        ...
        self.forecasts = {}
        for i in range(40):
            self.forecasts[i] = ForecastFrame(self.forecastsFrame,(c['light_bg'],c['dark_bg']),c,icons,bfont,mfont,sfont,efont)
...
class ForecastFrame(ctk.CTkFrame):
    def __init__(self,parent,fg_color,c,icons,bfont,mfont,sfont,efont):
        ...
        # populate frame
        self.populate(c,icons,bfont,mfont,sfont,efont)

        # place frame
        self.pack(expand=True,fill='x',pady=(0,6))
    
    def populate(self,c,icons,bfont,mfont,sfont,efont):
        self.forecastLabel = ctk.CTkLabel(self,font=sfont,text_color=(c['light_sfont'],c['dark_sfont']),text='forecast text')
        self.forecastLabel.pack(expand=True,padx=25,pady=25)
    ...
    def __init__(self):
        ...
        def update():
            ...
            if self.weather['cod']==200:
                ...
                self.side_frame.weatherTypeLabel.configure(font=self.mfont,text=self.weather['weather'][0]['description'].title())
...
Set {datetime.utcfromtimestamp(self.weather['sys']['sunset']+self.weather['timezone']).strftime('%H:%M')}''')
        ...
        def updateTime():
            if self.weather['cod']==200:
                ...
                self.side_frame.timeLabel.configure(text='[Regional Time]')

            self.side_frame.after(1000, updateTime)


        self.main_frame.colorModeButton.configure(command=toggleMode)
        self.side_frame.runButton.configure(command=update)
        updateTime()
