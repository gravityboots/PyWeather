...
Set {datetime.utcfromtimestamp(self.weather['sys']['sunset']+self.weather['timezone']).strftime('%H:%M')}''')
                ...
                for r in range(40):
                    strtime = datetime.fromtimestamp(self.forecast['list'][r]['dt']+self.weather['timezone']).strftime('%a %H:%M')
        ...
        def updateTime():  # constantly update local time, timezone and location display
            if self.weather['cod']==200:

                self.time = datetime.fromtimestamp(datetime.utcnow().timestamp()+self.weather['timezone'])
                self.side_frame.dayLabel.configure(text=self.time.strftime('%A %Z'))
